...
export default function Bagian3Form({ vcfId, canEdit, canFill, vcfData, onSuccess }: Props) {
  ...
  const mapExistingData = (items: CheckItem[], data: any) => {
    ...
    // 3. Map segel detail - IMPORTANT: Set SGK pemeriksaan to "Terpasang" or "Tidak Terpasang" if segel data exists
    const segelItem = items.find(i => i.kode === "SGK");
    if (data.segel_keluar) {
      ...
      if (segelItem) {
        ...
        setPemeriksaan(prev => ({
          ...prev,
          [segelItem.id]: isTerpasang ? "Terpasang" : "Tidak Terpasang"
        }));
      }
    }
  ...
  const validateForm = (): { valid: boolean; message?: string } => {
    ...
    // Catatan: SGK di Bagian 3 (WB Keluar) hanya verifikasi keberadaan segel (Terpasang/Tidak Terpasang)
    // Nomor segel untuk Loading diinput di Bagian 2 (WB Masuk), bukan di sini
    // Tidak ada validasi nomor segel di Bagian 3
      ...
      <div className="p-6 border border-slate-100 dark:border-white/5 rounded-3xl shadow-sm bg-white dark:bg-bg-card overflow-hidden">
        <div className="px-6 py-4 border-b border-border bg-slate-50/50 dark:bg-white/5 flex justify-between items-center">
          <div className="border-l-4 border-purple-500 pl-4">
            ...
            <p className="text-[10px] text-text-muted mt-0.5">Detail data pemeriksaan weighbridge saat keluar area</p>
          </div>
            ...
            <div className="mt-6 pt-6 border-t border-border grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {vcfData.segel_keluar && (
                <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                  <p className="form-label text-emerald-700 border-l-2 border-emerald-500 pl-2">Segel Keluar ({vcfData.segel_keluar.jumlah_segel} Unit)</p>
              ...
              {isUnloading && vcfData.segel_masuk && (
                <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                  <p className="form-label text-emerald-400 border-l-2 border-emerald-500 pl-2">Segel Masuk ({vcfData.segel_masuk.jumlah_segel} Unit) - dari Registrasi</p>
                  <div className="flex flex-wrap gap-2 mt-2">
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-28 lg:pb-0">
      ...
      <div className="glass-card p-3 md:p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-2 md:gap-4 text-xs">
          ...
          <span className="text-text-muted">•</span>
          <span className="font-bold uppercase text-purple-600 dark:text-purple-400">{activityType.replace("_", " ")}</span>
      ...
      <form id="vcf-bagian3-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-0.5">
          <div className="flex items-center justify-between border-l-4 border-purple-500 pl-3 mb-3">
            <div>
              <span className="text-[11px] font-black uppercase tracking-widest text-purple-500 block">Pemeriksaan Weighbridge Keluar</span>
              <span className="text-[10px] text-text-muted">Periksa semua item kelayakan keluar kendaraan</span>
            </div>
          ...
          {pemeriksaanItems.map((item) => {
            ...
            return (
              <div key={item.id} data-error={hasError ? "true" : undefined}>
                <div className={`px-4 py-3.5 rounded-2xl border border-l-4 bg-white dark:bg-white/[0.03] transition-all duration-200
                  ${hasError
                    ...
                    {isSelect ? (
                      <div className="flex gap-2 flex-wrap">
                        {options!.map((opt: string, idx: number) => {
                          ...
                          const isNegative = idx === options!.length - 1;
                          return (
                            <label key={opt} className={`
                              ...
                              ${isSelected
                                ...
                                : 'bg-transparent border-slate-200 dark:border-white/15 text-slate-500 hover:border-purple-400 hover:text-purple-500'
                              }`}>
                              <input type="radio" className="hidden" name={`pem-k-${item.id}`} checked={isSelected}
                      ...
                      <input type="text" className="form-input sm:max-w-[240px] bg-white dark:bg-white/5"
                        placeholder="Masukan hasil..." value={value || ""}
                  ...
                  {item.kode === "BTK" && value === "Ada" && (
                    <div className="mt-3 pt-3 border-t border-purple-500/10">
                      <label className="form-label text-purple-400">Sebutkan Jenis Beban (Keluar)</label>
                      <input type="text" className="form-input bg-purple-500/5 border-purple-500/20 focus:border-purple-500"
                        placeholder="..." value={jenisBeban} onChange={(e) => setJenisBeban(e.target.value)} />
                    </div>
                  )}

                  {/* Segel reference — show for both flows (read-only) */}
                  {item.kode === "SGK" && (
                    <div className="mt-3 pt-3 border-t border-amber-500/10 animate-slideDown space-y-2">
                      ...
                      {isLoading ? (
                        <p className="text-[11px] text-text-muted italic px-1">Nomor segel diinput oleh petugas Main Gate Keluar.</p>
                      ) : vcfData.segel_masuk ? (
                        <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/15">
                          <p className="text-[10px] text-amber-500/70 font-bold uppercase mb-2">Jumlah: {vcfData.segel_masuk.jumlah_segel} Unit</p>
                        ...
                        <p className="text-[11px] text-text-muted italic px-1">Tidak ada segel dari registrasi.</p>
                      )}
                    </div>
                  )}
                </div>
